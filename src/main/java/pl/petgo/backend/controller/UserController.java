...
@Validated
@Tag(name = "User Module", description = "Endpoints for managing users, their profiles, addresses, and wallets")
public class UserController {
    private final UserService users;

    @Operation(summary = "Get all users", description = "Returns a list of all users. Typically restricted to administrators.")
    @GetMapping
    ...
    @Operation(summary = "Get user addresses", description = "Returns all addresses associated with a specific user. " + "Accessible by the user themselves or an administrator.")
    ...
    @Operation(summary = "Get user wallet", description = "Retrieves wallet details for a specific user, including balance information.")
    ...
    @Operation(summary = "Add address to user", description = "Creates a new address and assigns it to the specified user.")
    @PostMapping(value = "/{id}/addresses", consumes = "application/json")
    @PreAuthorize("hasRole('ADMIN') or #id == principal.userId")
    public ResponseEntity<AddressResponse> addAddress(@AuthenticationPrincipal AppUserDetails principal, @PathVariable Long id, @Valid @RequestBody CreateAddressRequest address) {
    ...
    @Operation(summary = "Update user data", description = "Updates user profile data such as name, email, or other editable fields.")
